...
import com.tamnguyen.identityService.dto.request.auth.*;
...
import com.tamnguyen.identityService.repository.httpclient.OutboundIdentityClient;
import com.tamnguyen.identityService.repository.httpclient.OutboundUserClient;
...
public class AuthenticationService {
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        ...
        return AuthenticationResponse.builder().user(userMapper.toUserResponse(user)).accessToken(accessToken).refreshToken(refreshToken).build();
    }
    ...
    public TokenInfo generateToken(User user, String signerKey, long tokenExpiration) {
        ...
        Date expiryTime = new Date(Instant.ofEpochMilli(issueTime.getTime()).plus(tokenExpiration, ChronoUnit.MILLIS).toEpochMilli());

        JWTClaimsSet jwtClaimsSet = new JWTClaimsSet.Builder().subject(user.getUsername()).issuer("tamnguyen.com").issueTime(issueTime).expirationTime(expiryTime).jwtID(UUID.randomUUID().toString()).claim("scope", buildScope(user)).claim("userId", user.getId()).build();
