...
public class AuthenticationFilter implements GlobalFilter, Ordered {
    ...
    @NonFinal
    private final String[] publicEngPoints = {
            "/identity/auth/.*",
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        log.info(String.valueOf(isPublicEndpoint(exchange.getRequest())));
    ...
    private boolean isPublicEndpoint(ServerHttpRequest request) {
        return Arrays.stream(publicEngPoints).anyMatch(s -> request.getURI().getPath().matches(apiPrefix + s));
    }
